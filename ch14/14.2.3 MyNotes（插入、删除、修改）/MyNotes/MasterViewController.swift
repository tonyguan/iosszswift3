...
class MasterViewController: UITableViewController, NSURLConnectionDataDelegate {
    ...
    func startRequest()
    {
        ...
        if action == ActionTypes.QUERY {//查询处理
            post = String(format: "email=%@&type=%@&action=%@", "1293855300@qq.com", "JSON", "query")
            ...
            post = String(format: "email=%@&type=%@&action=%@&id=%@", "1293855300@qq.com", "JSON", "remove",id)
        }
    ...
    func reloadView(res : NSDictionary) {
        ...
        let resultCode = res.objectForKey("ResultCode") as! NSNumber
